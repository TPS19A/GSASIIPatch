...
def GeoCali(image, S2D_distance=260, horizontal_include=False, inner_include=True):
    # varieties
    vertical_factor   = {'260':[-7.30926706e-10, 4.24945779e-06, -7.71537925e-03, 4.32570758e+00],}
    horizontal_factor = {'260':[],}
    inner_fitout = np.linspace(-0.1, 0.1, 4096)
    pixelx = np.arange(4096)
    
    # apply vertical fit
    vertical_fitout = np.poly1d(vertical_factor[str(S2D_distance)])(pixelx)
    vertical_curved = pixelx+vertical_fitout
    app_im = np.array([np.interp(pixelx, vertical_curved, image[:,i]) for i in range(4096)], dtype=np.int32).T
        
    # apply horizontal fit
    if horizontal_include:
        horizontal_fitout = np.poly1d(horizontal_fitout)(pixelx)
        horizontal_curved = pixelx+horizontal_fitout
        app_im = np.array([np.interp(pixelx, horizontal_curved, app_im[i]) for i in range(4096)], dtype=np.int32)
